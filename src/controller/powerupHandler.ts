...
export default class PowerupHandler {
  ...
  private _powerupUpdate: {action: PowerupAction, powerup: Powerup, player: Player}[] = [];
  private _collisionHandler: CollisionHandler;
  private _maxPowerupAmount: number;

  constructor(players: Player[], avgTimeBetweenPowerups: number, maxPowerupAmount: number, collisionHandler: CollisionHandler) {
  ...
  private addPowerup(powerup: Powerup) {
    this._powerupUpdate.push({action: PowerupAction.SPAWN, powerup, player: null});
    this._powerups[powerup.id] = powerup;
  }

  public removePowerup(powerup: Powerup) {
    this._powerupUpdate.push({action: PowerupAction.REMOVE, powerup, player: null});
        ...
        if (this.isPointInCircle(powerup.position, powerup.radius, snake.head.endPoint, 5)){
          switch (powerup.type) {
            case PowerupType.PortalWalls:
              ...
              this._powerupUpdate.push({action: PowerupAction.APPLY, powerup, player: player});
              break;
